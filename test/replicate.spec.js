  ...
  describe('ipfs-log - Replication (' + IPFS + ')', function () {
    ...
    describe('replicates logs deterministically', function () {
      ...
      it('replicates logs', async () => {
        ...
        assert.strictEqual(values[0].payload, 'A1')
        assert.strictEqual(values[1].payload, 'B1')
        assert.strictEqual(values[2].payload, 'A2')
        assert.strictEqual(values[3].payload, 'B2')
        assert.strictEqual(values[99].payload, 'B50')
        assert.strictEqual(values[100].payload, 'A51')
        assert.strictEqual(values[198].payload, 'A100')
        assert.strictEqual(values[199].payload, 'B100')
      })
    })
  })
})
